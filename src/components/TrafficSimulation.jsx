...
const RenderCarsBottomAnimated = ({ count }) => {
  ...
  return (
    <div className="grid grid-cols-3 grid-flow-row auto-rows-auto gap-1 mr-20 mt-48">
      {carDivs}
    </div>
  );
};
...
const RenderCardBottom = ({ count }) => {
  ...
  return (
    <div
      className="grid grid-cols-3 grid-flow-row auto-rows-auto gap-1 mr-20 mt-48  "
  ...
  return (
   <div className="relative w-full max-w-2xl h-[32rem] mx-auto mt-8 bg-green-200 border border-gray-300 ">
      ...
      <div className={"absolute left-1/2 top-0 bottom-0 w-40 bg-gray-600  transform -translate-x-1/2 overflow-hidden"+((activeLightIndex==0 || activeLightIndex==2 )?" z-50":"") }>
        {/* cars top */}
      {activeLightIndex!=0 &&  <RenderCarsTop count={vehicle_data[2]} />}  
        { activeLightIndex==0 && <RenderCarsTopAnimated count={lightTimers[0]} /> }


        <div className="absolute left-1/2 top-0 bottom-0 w-1 bg-yellow-400 transform -translate-x-1/2 overflow-hidden"></div>
        {activeLightIndex!=2 && <RenderCardBottom count={vehicle_data[2]} />}
        {activeLightIndex==2 && <RenderCarsBottomAnimated count={lightTimers[2]} />}
      </div>

      {/* Horizontal road */}
      <div className="absolute top-1/2 left-0 right-0 h-40 bg-gray-600 transform -translate-y-1/2 ">
     
        {activeLightIndex!=3 && <RenderCarsleft count={vehicle_data[3]} />} 
        {activeLightIndex==3&&  <RenderCarsLeftAnimated count={lightTimers[3]} />}
        <div className="absolute top-1/2 left-0 right-0 h-1 bg-yellow-400 transform -translate-y-1/2 "></div>
        {activeLightIndex !=1&& <RenderCarsRighht count={vehicle_data[0]} />} 
    
        {activeLightIndex==1 && <RenderCarsRighhtAnimated count={lightTimers[1]} />} 
